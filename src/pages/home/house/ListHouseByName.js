...
import {searchHouse} from "../../../services/houseService";
...
const ListHouseByName = () => {
    const dispatch = useDispatch();
    let {name} = useParams()

    useEffect(() => {
        dispatch(searchHouse(name))
    }, []);
    const list = useSelector(state => {
        console.log("aaaaa",state.house.searchName)
        return state.house.searchName
    })
    return (
        <div>
            <div id="container" className="container-fluid">
                <div className="row">
                    {list && list.map((house, index)=>(
                        <div className="col-2">
...
export default ListHouseByName;